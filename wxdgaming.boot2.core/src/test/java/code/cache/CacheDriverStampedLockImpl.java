 ...
 **/
@Getter
public class CacheDriverStampedLockImpl<K, V> {
    ...
    private final AtomicReference<CacheDriverStampedLock<K, V>> coreCacheReference = new AtomicReference<>();
    private final AtomicReference<CacheDriverStampedLock<K, V>> heartCacheReference = new AtomicReference<>();
    private final AtomicReference<CacheDriverStampedLock<K, Hold>> outerCacheReference = new AtomicReference<>();
    ...
    private final Consumer3<K, V, CacheDriverStampedLock.RemovalCause> heartListener;
    private final Predicate3<K, V, CacheDriverStampedLock.RemovalCause> removalListener;

    @Builder
    public CacheDriverStampedLockImpl(String cacheName, int blockSize, Duration heartExpireAfterWrite, Duration expireAfterAccess, Duration expireAfterWrite, Function<K, V> loader, Consumer3<K, V, CacheDriverStampedLock.RemovalCause> heartListener, Predicate3<K, V, CacheDriverStampedLock.RemovalCause> removalListener) {
    ...
    private void init() {
        ...
        coreCacheReference.set(
                CacheDriverStampedLock.<K, V>builder()
                        .loader(loader)
                        .blockSize(blockSize)
        ...
        heartCacheReference.set(
                CacheDriverStampedLock.<K, V>builder()
                        ...
                        .removalListener((k, v, removalCause) -> {
                            if (removalCause != CacheDriverStampedLock.RemovalCause.EXPIRE) return true;
        ...
        outerCacheReference.set(
                CacheDriverStampedLock.<K, Hold>builder()
                        .blockSize(blockSize)
    ...
    public void put(K k, V v) {
        outerCacheReference.get().invalidate(k, CacheDriverStampedLock.RemovalCause.EXPLICIT);
        heartCacheReference.get().invalidate(k, CacheDriverStampedLock.RemovalCause.EXPLICIT);
    ...
    public void invalidate(K k) {
        outerCacheReference.get().invalidate(k, CacheDriverStampedLock.RemovalCause.EXPLICIT);
        heartCacheReference.get().invalidate(k, CacheDriverStampedLock.RemovalCause.EXPLICIT);
        coreCacheReference.get().invalidate(k, CacheDriverStampedLock.RemovalCause.EXPLICIT);
    }
