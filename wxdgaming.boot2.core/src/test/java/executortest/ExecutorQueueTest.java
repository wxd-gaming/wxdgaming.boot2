...
public class ExecutorQueueTest {
    ...
    public void abortPolicy() {
        ...
        ExecutorQueue executorQueue = new ExecutorQueue(executorServicePlatform, 10, QueuePolicyConst.AbortPolicy);
    ...
    public void discardPolicy() {
        AtomicInteger count = new AtomicInteger(0);
        ExecutorServicePlatform executorServicePlatform = ExecutorFactory.create("t", 10);
        ExecutorQueue executorQueue = new ExecutorQueue(executorServicePlatform, 10, QueuePolicyConst.DiscardPolicy);
        for (int i = 0; i < 1000; i++) {
            int finalI = i;
            executorQueue.execute(() -> {
                // log.info("执行任务 {}", finalI);
    ...
    public void waitPolicy() throws InterruptedException {
        AtomicInteger count = new AtomicInteger(0);
        ExecutorServicePlatform executorServicePlatform = ExecutorFactory.create("t", 10);
        ExecutorQueue executorQueue = new ExecutorQueue(executorServicePlatform, 10, QueuePolicyConst.WaitPolicy);
        for (int i = 0; i < 1000; i++) {
            int finalI = i;
            executorQueue.execute(() -> {
                // log.info("执行任务 {}", finalI);
